import { AvailabilityStatus, BookingStatus, UserRoles, type Booking, type User } from "../../../generated/prisma/client";
...
const createBooking = async (data : Booking, studentId : string) => {
    ...
    const [startHour, startMinute] : number[] = startTime.split(":").map(Number) as [number, number];
...
const updateBookingStatus = async (bookingId : string, status : BookingStatus, user : User, tutorId ?: string | null) => {
...
export const bookingService = {createBooking, getAllBookings, updateBookingStatus}